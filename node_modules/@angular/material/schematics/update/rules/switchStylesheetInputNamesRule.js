...
class Rule extends tslint_1.Rules.AbstractRule {
    apply(sourceFile) {
        return this.applyWithWalker(new SwitchStylesheetInputNamesWalker(sourceFile, this.getOptions()));
    }
}
exports.Rule = Rule;
class SwitchStylesheetInputNamesWalker extends component_walker_1.ComponentWalker {
        ...
        component_data_1.inputNames.forEach(name => {
            if (!name.whitelist || name.whitelist.css) {
                ...
                this.createReplacementsForOffsets(node, name, literal_1.findAll(stylesheetContent, bracketedName.replace)).forEach(replacement => {
                    replacements.push({
                        message: `Found deprecated @Input() "${chalk_1.red(name.replace)}" which has been renamed` +
                            ` to "${chalk_1.green(name.replaceWith)}"`,
                        replacement
                    });
                });
            }
...
exports.SwitchStylesheetInputNamesWalker = SwitchStylesheetInputNamesWalker;
//# sourceMappingURL=switchStylesheetInputNamesRule.js.map